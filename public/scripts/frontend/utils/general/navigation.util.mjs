...
export default class Navigation {
    static navigateToNotes() {
        ...
        const notesState = new NotesState('', 'notes', '', '', '', 0, state.colorPalette);
        notesState.replaceWindowHistoryState();
    }

    static navigateToCreate() {
        const { state } = window.history;
        const notesState = new NotesState('', 'create', '', '', '', 0, state.colorPalette);
        notesState.replaceWindowHistoryState();
    }
    ...
    static navigateToEdit($event) {
        ...
        if (attributes && attributes['note-id'] && attributes['note-id'].value) {
            ...
            const notesState = new NotesState('', 'edit', '', '', '', noteId, state.colorPalette);
            notesState.replaceWindowHistoryState();
        }
    }
}
